...
**If this environment has a dedicated Outputs/Files panel separate from the chat itself** (e.g. Claude's Cowork-style task view, which shows "Outputs" and "Context" panels alongside the conversation): a file written to that environment's outputs location does not automatically mean the tutor knows it's there or what to click. Never end a turn with only a vague summary like "both files were written to the outputs folder, already delivered, no separate send needed" — a first-time tutor won't know what that means or where to look. Instead, explicitly:
- Name every file you created, exactly as written (e.g. "maria-lesson-1.docx" and "maria-lesson-1-handout.docx")
...
- Don't assume the panel being visible is enough — a tutor skimming chat text needs the file names and the click target spelled out in the same message, not left implicit
